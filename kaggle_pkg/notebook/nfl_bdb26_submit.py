...
def _find_dir(env, marker, default):
    """Locate a mounted input dir by a marker file, robust to slug/mount quirks."""
    d = os.environ.get(env)
    if d and os.path.exists(os.path.join(d, marker)):
        return d
    if os.path.exists(os.path.join(default, marker)):
        return default
    hits = glob.glob(f"/kaggle/input/**/{marker}", recursive=True)
    if hits:
        return os.path.dirname(hits[0])
    raise FileNotFoundError(f"could not locate {marker} under /kaggle/input; "
                            f"contents: {os.listdir('/kaggle/input')}")


MODEL_DIR = _find_dir("MODEL_DIR", "prepare.py", "/kaggle/input/nfl-bdb26-models")
COMP_DIR = _find_dir("COMP_DIR", "kaggle_evaluation",
                     "/kaggle/input/nfl-big-data-bowl-2026-prediction")
print(f"MODEL_DIR={MODEL_DIR}  COMP_DIR={COMP_DIR}", flush=True)
...
# each entry is (model, config_dict, stats_dict) — carry per-model config+stats
# so an ensemble of models trained on different folds (different z-score stats)
# or even different architectures each runs under its OWN normalization.
MODELS = []
_all = sorted(glob.glob(f"{MODEL_DIR}/**/*.pt", recursive=True))
# prefer the final ensemble (ens_*.pt); fall back to any checkpoint
_ckpts = [f for f in _all if os.path.basename(f).startswith("ens_")] or _all
for f in _ckpts:
...
def play_arrays(test: pl.DataFrame, test_input: pl.DataFrame):
    ...
    for nid, pg in g.groupby("nfl_id"):
        ...
        # n_out must come from test_input's num_frames_output (set for ALL
        # players), exactly like prepare.preprocess. Deriving it from the test
        # dataframe leaves n_out=0 for every unscored player — a value the
        # model only saw on padding slots — and cost 0.052 RMSE on the real
        # test (parity test 2026-07-03).
        if "num_frames_output" in g.columns:
            n_out[0, i] = int(r["num_frames_output"])
        else:
...
def predict(test: pl.DataFrame, test_input: pl.DataFrame) -> pl.DataFrame:
    ...
    with torch.no_grad():
        acc = None
        for m, cfg, stats in MODELS:
            # forward + make_batch read module-level config globals (T_USED,
            # DIM1, ...) so re-apply this model's config before running it
            for k, v in cfg.items():
                setattr(T, k, v)
    ...
    # 1st-place inference clips predictions to the field (their lines 1002-3)
    preds[..., 0] = np.clip(preds[..., 0], 0.0, prepare.FIELD_X)
    preds[..., 1] = np.clip(preds[..., 1], 0.0, prepare.FIELD_Y)
